...
\begin{itemize}
  \item The original RF-DNA pipeline~\cite{alla2024pla} reports ADR $\approx \rnfOrigADR$ under the paper protocol. When the evaluation is aligned to the same group-aware, fold-aware protocol used for the DL models, the RnF+ANOVA pipeline achieves ADR $\approx \rnfADR$ (approximately 40 percentage points lower), isolating three protocol differences between the two evaluations.
  \item Under this common evaluation protocol, the RnF model and all three DL models
...
\begin{itemize}
  \item \textbf{Protocol alignment.} Identified and isolated three methodological differences between the original RF-DNA pipeline and the DL evaluation, providing per-difference ADR measurements to quantify their individual impact.
  \item \textbf{Rigorous DL baseline.} First evaluation of MLP, GRU, and BiGRU architectures on this dataset under a strictly group-aware, transient-aware protocol across five seeds and LOTO.
